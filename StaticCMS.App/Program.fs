...
module MetadataTest =
    ...
    let run _ =
        ...
        let (r1, md1) =
            FDOM.Core.Parsing.Parser.ParseLinesAndMetadata(lines1)

        let (r2, md2) =
            FDOM.Core.Parsing.Parser.ParseLinesAndMetadata(lines2)
...
module ArticulusTest =
    ...
    let run _ =
        ...
        let fragmentData =
            ({ Title = "News"
               Items = fds |> List.truncate 3 }: FragmentData)
...
module PeepsTest =
    ...
    let root =
        "C:\\ProjectData\\static_cms\\sites\\Peeps"

    let renderedPath =
        Path.Combine(root, "rendered")
    ...
    let initialize (store: StaticStore) =
        ...
        store.AddSitePlugin("peeps", "articulus", File.ReadAllText("C:\\ProjectData\\static_cms\\sites\\Peeps\\plugins\\articulus\\config.json"))
        
    
    let copyResources _ =
        let cssPath =
            Path.Combine(resourcesPath, "css")

        let jsPath =
            Path.Combine(resourcesPath, "js")

        let imgPath =
            Path.Combine(resourcesPath, "img")

        let cssOutPath =
            Path.Combine(renderedPath, "css")

        let jsOutPath =
            Path.Combine(renderedPath, "js")

        let imgOutPath =
            Path.Combine(renderedPath, "img")
    ...
    let runArticulus (staticStore: StaticStore) (versionRef: string) =
        let articulusPath =
            Path.Combine(pluginsPath, "articulus")

        let store =
            ArticulusStore.Create(Path.Combine(articulusPath, "store.db"))
        ...
        let result =
            renderAll store values template renderedPath "news"
    ...
    let runPlugins (staticStore: StaticStore) (versionRef: string) =
        ...
        match Faaz.ScriptHost.eval<string> "C:\\Users\\44748\\Projects\\StaticCMS\\StaticCMS.PluginScripts\\Articulus.fsx" $"""Articulus.run "C:\\ProjectData\\static_cms\\static_store-test.db" "peeps" """ fsi
            with
        | Ok r ->
    ...
    let run _ =
        ...
        let store =
            StaticStore.Create("C:\\ProjectData\\static_cms\\static_store-test.db")
...
let store =
    StaticStore.Create("C:\\ProjectData\\static_cms\\static_store.db")
